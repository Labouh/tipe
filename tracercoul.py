...
def duree_de_vie(lr,lv,lb,tmax,n, save=True):
    
    datar = np.zeros((len(lr),tmax//n +1), dtype = 'int')
    datav = np.zeros((len(lv),tmax//n +1), dtype = 'int')
    datab = np.zeros((len(lb),tmax//n +1), dtype = 'int')
    datatot = np.zeros((3*len(lr),tmax//n +1), dtype = 'int')
    datar[0],datav[0],datab[0],datatot[0] = 2,2,2,2
    for r in lr :
        dr = r[1] - r[0]
        for p in range(r[0]//n,r[1]//n +1):
            if r[1] < tmax:
    ...
    for b in lb :
        db = b[1] - b[0]
        for p in range(b[0]//n,b[1]//n +1):
            datab[datab[0,p],p] = db
            datab[0,p] += 1
            datatot[datatot[0,p],p] = db
            datatot[0,p] += 1
    print('bleue done')
    for i in range(tmax//n +1):
        if datar[0,i] != 2 :
...
def trace_graph(liste,tmax,n, save = True):
    T = np.arange(0,tmax//n +1)
    lr,lv,lb = separe(liste)
    mr,mv,mb,mtot = duree_de_vie(lr,lv,lb,tmax,n, save)
...
def trace_graph_data(datar, darav, datab, datatot, tmax, n):
    T = np.arange(0,tmax//n +1)
